// Nueva Venta - Intelligent Pro Terminal 2025 (Z-Index & Layout Fix)
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import {
    Search, Plus, Trash2, User, ShoppingCart,
    Check, X, Zap, ArrowLeft, Target, ChevronRight,
    LayoutGrid, Activity, Package, BadgePercent, FileText, Ban
} from 'lucide-react';
...
const NuevaVenta = () => {
    ...
    const handleAutoAdd = (producto, cantidad, precio) => {
        if (!producto) return;
        if (producto.stock <= 0) {
            showWarningAlert('Sin Stock', `El producto "${producto.descripcion}" no tiene stock.`);
            return;
        }
        const existe = items.find(i => i.id === producto.id);
        const cantidadTotal = existe ? existe.cantidad + cantidad : cantidad;
        if (cantidadTotal > producto.stock) {
            showWarningAlert('Stock Insuficiente', `Disponible: ${producto.stock}`);
            return;
        }
        if (existe) {
    ...
    const recalcularPrecios = (nuevoMedio) => {
        ...
        setItems(prev => prev.map(item => {
            ...
            return { ...item, precio: nuevoPrecio, subtotal: item.cantidad * nuevoPrecio };
        }));
    };
    ...
    const seleccionarCliente = (c) => {
        ...
        if (c.condicion_fiscal === 'RI') {
            setDiscriminarIVA(true);
        } else {
    ...
    const cancelarVenta = () => {
        showConfirmationAlert(
            '¿Cancelar Venta?',
            'Se borrarán todos los items cargados del carrito actual.',
            'SÍ, CANCELAR',
            'danger',
            {
            ...
            if (result.isConfirmed) {
                ...
                setCliente(null);
                limpiarCamposEntrada();
                showSuccessAlert('Venta Cancelada', 'La terminal ha sido reiniciada.').then(() => {
                    setTimeout(() => {
                        clienteInputRef.current?.focus();
                        clienteInputRef.current?.select();
                    }, 50);
                });
            }
        });
    ...
    const cambiarCantidad = (id, nueva) => {
        if (nueva < 1) return;
        setItems(items.map(i => i.id === id ? { ...i, cantidad: nueva, subtotal: nueva * i.precio } : i));
    };
        ...
        try {
            const res = await fetch('/api/ventas/guardar/', {
                ...
                body: JSON.stringify({
                    ...
                    items: items.map(i => ({
                        id: i.id, cantidad: i.cantidad, precio: i.precio, subtotal: i.subtotal,
                    ...
                    total_general: items.reduce((a, b) => a + b.subtotal, 0) + (paymentData.percepcion_iva || 0) + (paymentData.percepcion_iibb || 0),
                    medio_pago: paymentData.metodo_pago,
            ...
            const data = await res.json();
            if (data.ok) {
                showSuccessAlert('Venta Exitosa', `Operación #${data.venta_id} procesada.`);
                setItems([]); setCliente(null); setMostrarModalPago(false);
                setTimeout(() => clienteInputRef.current?.focus(), 500);
    ...
    return (
        <div className="p-3 lg:p-4 max-w-[1900px] mx-auto space-y-3 animate-in fade-in duration-500 h-full bg-slate-50 flex flex-col overflow-hidden relative">
            ...
            <div className="flex items-center gap-4 bg-white p-3 px-5 rounded-2xl border border-neutral-200 shadow-sm flex-shrink-0 relative z-[200]">
                ...
                {/* Cliente Selector - Overlays everything below */}
                <div className="flex-1 max-w-sm relative overflow-visible">
                    <input
                        ...
                        className="w-full pl-4 pr-4 py-2 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all text-sm font-bold"
                    />
                                ...
                                <div key={c.id} onClick={() => seleccionarCliente(c)} className={cn("px-4 py-3 cursor-pointer rounded-lg text-xs font-bold transition-all", idx === sugerenciaClienteActiva ? "bg-primary-600 text-white shadow-lg translate-x-1" : "hover:bg-neutral-50")}>
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className={cn("text-[8px] px-1.5 py-0.5 rounded ml-2", idx === sugerenciaClienteActiva ? "bg-white/20" : "bg-neutral-100 text-neutral-500")}>
                                            {c.condicion_fiscal_display || 'C.F.'}
                                        </span>
                                    </div>
                                </div>
                            ))}
                ...
                {cliente && (
                    <div className="flex items-center gap-3 bg-primary-600 text-white px-4 py-2 rounded-xl animate-in zoom-in-95 shadow-lg shadow-primary-600/20">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[8px] font-bold text-primary-200 uppercase mt-0.5">{cliente.condicion_fiscal_display}</span>
                        </div>
                ...
                <div className="flex items-center gap-2 px-4 border-l border-neutral-100">
                    ...
                    >
                        ...
                        DISCRIMINAR IVA {discriminarIVA ? 'ON' : 'OFF'}
                    </button>
                    {items.length > 0 && (
                        <button
            ...
            <div className="flex-1 bg-white rounded-3xl shadow-premium border border-neutral-200 flex flex-col overflow-hidden relative z-[50]">
                ...
                <div className="px-6 py-4 border-b border-neutral-200 bg-neutral-50/50 flex-shrink-0 relative z-[100] overflow-visible">
                    <div className="grid grid-cols-12 gap-4 items-end overflow-visible">
                        ...
                        <div className="col-span-2">
                            <button
                                onClick={() => handleAutoAdd(productoSeleccionado, parseFloat(inputCantidad), parseFloat(inputPrecio))}
                ...
                {/* Main Table Area - Low Z-Index */}
                <div className={cn("flex-1 min-h-0 bg-white relative z-10 flex flex-col", items.length > 0 ? "overflow-y-auto shadow-inner" : "overflow-hidden")}>
                ...
                <div className="p-4 px-8 bg-neutral-950 text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-[0_-10px_40px_rgba(0,0,0,0.3)] flex-shrink-0 relative z-20 border-t border-white/5">
                    ...
                    >
                        {guardando ? "PROCESANDO" : "CONFIRMAR PAGO"} <ChevronRight size={24} strokeWidth={4} className="group-hover:translate-x-2 transition-transform" />
                    </button>
                </div>
            </div>
            ...
            <PaymentModal
                ...
                onConfirm={guardarVenta} onMethodChange={recalcularPrecios}
                total={totalGeneral} mode="sale" clientName={cliente?.nombre}
