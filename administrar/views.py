...
from .models import (
    Producto, Proveedor, Marca, Rubro, Categoria,
...
def api_bitacora_actividades(request):
    ...
    try:
        ...
        # Generar métricas estadísticas expertas
        from django.db.models import Count
        metrics = {}
        try:
            # Stats by module
            top_modules = list(queryset.values('module').annotate(count=Count('id')).order_by('-count')[:3])
            # Stats by action type
            top_actions = list(queryset.values('action_type').annotate(count=Count('id')).order_by('-count')[:3])
            metrics = {
...
def api_dashboard_stats(request):
    ...
    data = {
        'kpi': {
            ...
            'cantidad_pedidos_hoy': Pedido.objects.filter(fecha__range=(start_of_day, end_of_day)).count(),
        },
